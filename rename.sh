...
for repo in ./*; do #This looks at every file in the current directory
  ...
  git ls-remote --exit-code --heads  git@github.com:$GITHUBUSER/$CURRENT_DIR_NAME.git $BRANCH 
  ...
  then
    ...
    if [ $? == 0 ] # I think the line above asks a question, if we have a local branch w the target name,
    then # and then the $? is the answer to the question.
      ...
      git branch -m master base && git push -u $REMOTE base # TODO: what if this fails? like for permissions? conflicts?
    fi
  else
